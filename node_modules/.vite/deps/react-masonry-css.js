...
//#region node_modules/react-masonry-css/dist/react-masonry-css.module.js
...
var Masonry = class extends import_react.Component {
	constructor(props) {
		super(props);
	...
	reCalculateColumnCountDebounce() {
		...
		if (window.cancelAnimationFrame) window.cancelAnimationFrame(this._lastRecalculateAnimationFrame);
	...
	reCalculateColumnCount() {
		...
		if (typeof breakpointColsObject !== "object") breakpointColsObject = { default: parseInt(breakpointColsObject) || DEFAULT_COLUMNS };
		let matchedBreakpoint = Infinity;
		let columns = breakpointColsObject.default || DEFAULT_COLUMNS;
		for (let breakpoint in breakpointColsObject) {
			const optBreakpoint = parseInt(breakpoint);
			if (optBreakpoint > 0 && windowWidth <= optBreakpoint && optBreakpoint < matchedBreakpoint) {
				matchedBreakpoint = optBreakpoint;
				columns = breakpointColsObject[breakpoint];
			}
		}
		columns = Math.max(1, parseInt(columns) || 1);
		if (this.state.columnCount !== columns) this.setState({ columnCount: columns });
	}
	itemsInColumns() {
		const currentColumnCount = this.state.columnCount;
		const itemsInColumns = new Array(currentColumnCount);
		const items = import_react.Children.toArray(this.props.children);
	...
	renderColumns() {
		...
		const columnAttributes = _objectSpread(_objectSpread(_objectSpread({}, column), columnAttrs), {}, {
			style: _objectSpread(_objectSpread({}, columnAttrs.style), {}, { width: columnWidth }),
			className
		});
		return childrenInColumns.map((items, i) => {
			return /*#__PURE__*/ import_react.createElement("div", _extends({}, columnAttributes, { key: i }), items);
		});
	}
	...
	render() {
		const _this$props = this.props, { children, breakpointCols, columnClassName, columnAttrs, column, className } = _this$props, rest = _objectWithoutProperties(_this$props, [
			"children",
			"breakpointCols",
			"columnClassName",
			"columnAttrs",
		...
		return /*#__PURE__*/ import_react.createElement("div", _extends({}, rest, { className: classNameOutput }), this.renderColumns());
	}
};
